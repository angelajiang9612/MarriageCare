...
gen severhad=1 if r1cancre==1 | r1lunge==1 | r1hearte==1 | r1stroke==1	//spouse ever had health prob before the 1st wave
...
drop redflag
//dependent variable
gen dead=1 if rmstat==6 
replace dead=2 if rmstat==7	//seperated or widowed
gen redflag=1 if dead!=.
bysort hhidpn (wave): replace redflag=1 if redflag[_n-1]==1
replace redflag=0 if redflag==.
