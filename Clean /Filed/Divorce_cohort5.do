...
sort h7hhid
quietly by h7hhid:  gen dup = cond(_N==1,0,_n)
drop if dup>1 


drop s* //drop the spouse variables 

keep hhidpn h7hhid hacohort *mstat *mpart *mrct *mstath *mdiv *iwstat *agey_e //keep the variables of interest, so that it doesn't become too slow

reshape long r@mstat r@mpart r@mrct r@mstath r@mdiv r@iwstat r@agey_e, i(hhidpn) j(wave)

xtset hhidpn wave

keep if hacohort ==5 //EBB

by hhidpn (wave), sort: keep if rmstat[7]==1 | rmstat[7]==2 //keep the initially married 
